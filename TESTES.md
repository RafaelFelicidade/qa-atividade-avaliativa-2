...
- **Forçar Variáveis de Teste:** Atualizamos o arquivo `phpunit.xml` substituindo as tags `<env>` por `<server>` no bloco `<php>`, garantindo que as variáveis do sistema (como as do container Docker que rodam como `local` e `mysql`) não sobrescrevam o ambiente de teste. Isso solucionou definitivamente as falhas de segurança (erro CSRF 419) sem precisar passar variáveis via linha de comando.
